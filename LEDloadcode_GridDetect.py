...
# Latched PSU detection
# Once any LED current goes above this threshold, psu_on stays True forever.
PSU_CURRENT_THRESHOLD = 0.01
...
print("Latched current-based PSU detection")
print("Initial P_demand = {:.3f} W".format(P_demand))
print("PSU_CURRENT_THRESHOLD = {:.3f} A".format(PSU_CURRENT_THRESHOLD))
print("")
...
while True:
    ...
    if timer_elapsed == 1:
        ...
        if count >= POWER_CONTROL_INTERVAL:
            ...
            # Latched PSU detection
            #
            # Initially psu_on = False.
            #
            # If any measured LED current goes above the threshold once,
            # psu_on becomes True and stays True.
            #
            # This means turning on the PSU/sink resistor will not trigger the
            # latch unless actual LED current is measured above threshold.
            # ---------------------------------------------------------------
            current_detected = (
                abs(ired) > PSU_CURRENT_THRESHOLD or
                abs(iyel) > PSU_CURRENT_THRESHOLD or
                abs(igrn) > PSU_CURRENT_THRESHOLD
            )
            ...
            # Before actual LED current is detected, keep the setpoint at zero.
            if not psu_on:
                current_setpoint = 0.0

            # Normal power control only starts after the latch has triggered.
            if startup_detected:
