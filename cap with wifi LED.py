...
print("P URL: http://" + PICO_IP + "/p")
print("Va URL: http://" + PICO_IP + "/va")
...
# -----------------------------------------------------------------------------
# Web telemetry variables - ADDED ONLY
# -----------------------------------------------------------------------------
...
p = 0 #power
p_ref = 0 # power reference for the CL modes
p_err = 0 # power error
p_err_int = 0 # power error integral
p_pi_out = 0 # Output of the power PI controller
...
# -----------------------------------------------------------------------------
# Web request handler - ADDED ONLY
# -----------------------------------------------------------------------------

def handle_web_request():
    global p_fetch, va, vb, iL, p, duty
    ...
    try:
        ...
        if "GET /p " in request or "GET /p?" in request:
            body = str(p_fetch)

        elif "GET /pout " in request or "GET /pout?" in request:
            body = str(p_fetch)

        elif "GET /va " in request or "GET /va?" in request:
            body = str(va)

        elif "GET /data " in request or "GET /data?" in request:
            body = (
                '{{"p": {}, "va": {}, "vb": {}, "iL": {}, "duty": {}}}'
            ).format(
                p_fetch,
            ...
            body = "HELLO FROM PICO. Use /p for power, /va for capacitor voltage, or /data for JSON."
...
while True:
    ...
    if timer_elapsed == 1: # This is executed at 1kHz
        ...
        p_err_int = saturate(p_err_int, max_pwm/300, min_pwm/300) # saturate the integral error
